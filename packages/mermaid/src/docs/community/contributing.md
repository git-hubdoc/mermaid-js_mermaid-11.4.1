...
Just as important, the tests act as *specifications:* they specify what the code does (or should do).
...
If the users have no way to know that things have changed, then you haven't really *fixed* anything for the users; you've just added to making Mermaid feel broken.
...
If you want to propose changes to how the documentation is *organized*, such as adding a new section or re-arranging or renaming a section, you must update the **sidebar navigation**, which is defined in [the vitepress config](../.vitepress/config.ts). The same goes to **topbar**.
